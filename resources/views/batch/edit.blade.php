...
@section('content')
@include('users.partials.header', ['title' => __('Edit Batch')])
                ...
                <div class="card-header bg-white border-0">
                    <div class="row align-items-center">
                        ...
                        <div class="col-4 text-right">
                            <a href="{{ route('batch.index') }}" class="btn btn-sm btn-warning">{{ __('View All
                                Batches') }}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('batch.update', $batch) }}" autocomplete="off">
                        @csrf
                        @method('put')
                        ...
                        <div class="pl-lg-4">
                            <div class="form-group{{ $errors->has('year') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="year" id="input-year"
                                    ...
                                    placeholder="{{ __('Year') }}" value="{{$batch->year}}" required autofocus>
                            ...
                            <div class="text-center">
                                <button type="submit" class="btn btn-success mt-4 px-4">{{ __('Edit Batch') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
