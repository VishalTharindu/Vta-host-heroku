            ...
            <div class="card shadow">
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col-10">
                             <h3 class="mb-10">{{ __('Examination Payment') }}</h3>   
                        </div>
                        <div class="col-2">
                            @isset($recount)
                ...
                <div class="card-body">
                    <div>
                        <form method="post" action="{{ route('exmpayment.creates') }}" autocomplete="off">
                            @csrf
                    ...
                    <form action="{{route('exmpayment.store')}}" method="post"  autocomplete="off">
                        @csrf
                        <div class="mr-3 ml-3 mt-4">
                            <div class="table-responsive">
                                <table class="table align-items-center table-flush" id="datatable-buttons">
                                    ...
                                    @isset($trainees)
                                        @foreach ($trainees as $data)
                                            ...
                                            $unpaiedtrainees = App\ExaminationPayment::unpaiedTrainess($data->id);

                                            @endphp
                                            @if(($unpaiedtrainees == 1) && ($status == 1) )
                                                <tr>
                                ...
                                <div class="text-center my-3">
                                    <button type="submit" class="btn btn-success mt-4 px-4">{{ __('Save Record') }} </button>
                                </div>
                            </div>
                        </div>                      
                    </form>                  
