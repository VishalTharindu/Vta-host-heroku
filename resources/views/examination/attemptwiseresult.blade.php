    ...
    <div class="row">
        <div class="col">
            <div class="card shadow">
                            ...
                            @if(isset($courses))
                                @isset($requiredCourse)
                                    @foreach($courses as $course)
                                        ...
                                        <h3 class="mb-10">{{ __('Attempts Result : ') }}<span class="text-primary">{{ $course->course_name }}</span></h3>
                                        @endif
                                    @endforeach
                                @endisset
                             @else
                             <h3 class="mb-10">{{ __('Attempts Result') }}</h3>   
                            @endisset
                        </div>
                        <div class="col-2">
                            @isset($recount)
                ...
                <div class="card-body">
                    <div>
                        <form method="post" action="{{ route('trainee.exam.attempt.result') }}" autocomplete="off">
                            @csrf
                                        ...
                                        @foreach($trainees as $trainee)
                                            ...
                                            @foreach ($traineeResult as $data)
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <div class="dropdown">
                                                            ...
                                                            @foreach($subjects as $subject)
                                                                @php                                                              
                                                                    $score = App\Mark::traineeAttemptWiseConfidentLevel($trainee->id, $subject->id, $data->attempt);
                                                                @endphp
                                                    ...
                                                    @if($data->status == 0)
                                                    <td class="text-danger">Not Confident</td>
                                                    @else
    ...
    $(document).ready(function() {
        $('#datatable-custom').DataTable({
            ...
            buttons: [{
                    ...
                    title:
                    @if (isset($trainees))
                        'Trainee Attempts Written Exam Results'
                    @else
                        'Trainee Attempts Written Exam Results'
                    @endif,
                },
