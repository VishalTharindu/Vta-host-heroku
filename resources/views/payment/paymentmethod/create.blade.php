    ...
    <div class="row">
        <div class="col-xl-12 order-xl-1">
            <div class="card bg-secondary shadow">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('funds.store') }}" autocomplete="off">
                        @csrf
                                ...
                                <div class="col-md-12">
                                    <div class="form-group{{ $errors->has('full_name') ? ' has-danger' : '' }}">
                                        ...
                                        <textarea name="discription" id="name_with_initials"
                                            ...
                                            value="{{ old('name_with_initials') }}" required autofocus>  </textarea>
                                        @if ($errors->has('full_name'))
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $errors->first('full_name') }}</strong>
                                        </span>
    ...
    <script type='text/javascript'>
        $(document).ready(function() {
            ...
            $(document).on('click', ".add_form_field", function(e) {
                ...
                if (x < max_fields) {
                    x++;
                    $(wrapper).append('<div class="form-group{{$errors->has('full_name') ? ' has-danger' : ''}}" id="toremove"> <label class="form-control-label" for="input-criteria">{{__('Other Criteria')}}</label> <div class="input-group control-group increment"> <input type="text" name="criteria[]" class="form-control form-control-alternative{{$errors->has('name_with_initials') ? ' is-invalid' : ''}}" placeholder="{{__('Enter Criteria')}}" value="{{old('name_with_initials')}}" autofocus> <div class="input-group-btn"> <div class="my-2"></div><button class="btn btn-sm btn-danger delete" type="button"><i class="glyphicon glyphicon-remove"></i> Remove</button> </div></div>@if ($errors->has('full_name')) <span class="invalid-feedback" role="alert"> <strong>{{$errors->first('full_name')}}</strong> </span> @endif </div>'); //add input box
                } else {
