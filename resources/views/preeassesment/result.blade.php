    ...
    <div class="row">
        <div class="col">
            <div class="card shadow">
                            ...
                            @if(isset($courses))
                                @isset($requiredCourse)
                                    @foreach($courses as $course)
                                        ...
                                        <h3 class="mb-10">{{ __('Pre Assesstment Result : ') }}<span class="text-primary">{{ $course->course_name }}</span></h3>
                                        @endif
                                    @endforeach
                                @endisset
                             @else
                ...
                <div class="card-body">
                    <div>
                        <form method="post" action="{{ route('preassesment.trainee.finel.result') }}" autocomplete="off">
                            @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                @isset($subjects)
                                    ...
                                    @php
                                        $subjectfail = App\PreAssesmentMark::calculateSubjectFailPresentage($subject->id)

                                    @endphp

                                    @php
                                        $subjectpass = App\PreAssesmentMark::calculateSubjectPassPresentage($subject->id)
                                    @endphp
                                    <div class="col-md-4">
                                ...
                                <table class="table align-items-center table-flush" id="datatable-buttons">
                                    ...
                                    @isset($trainees)
                                        @foreach ($trainees as $data)
                                            @php
                                            $pass = App\PreAssesmentRsult::calculatePassPrecentage($requiredCourse, $requiredBatch);

                                            $status = App\PreAssesmentRsult::traineeCurrentStatus($data->id);

                                            $passfailstatus = App\PreAssesmentRsult::traineePassFailStatus($data->id);
